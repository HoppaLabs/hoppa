...
export const GALLERY: readonly Example[] = [
  ...
  // that are somebody.
  //
  // Three inks is not much to say "knight" with, and the first pair said it
  // badly: a grey lump with a dark hole for a visor, and a face-coloured
  // triangle with a bump on top. What fixed both was spending the LIGHT ink on
  // one bright thing rather than on shading -- gold. A gold plume and a gold
  // cross carry the knight; a gold crown, a gold sash and a gold hem carry the
  // princess. The rest of each is silhouette and dark lines: pauldrons, a
  // belt, greaves; hair framing a face, folds down a gown.
  {
    name: "knight",
    sub: [3, 28, 1],
    rows: [
