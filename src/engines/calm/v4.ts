// calm/4: the garden, with a body.
//
// A copy of calm/3, not an edit of it (hard rule 3): every garden link anybody
// has sent pins calm/3 or earlier and has to keep replaying under exactly the
// rules it was beaten under.
//
// What is added is roam/9's movement, unchanged and for the same reason. Up to
// calm/3 a step was `x += speed * direction` -- position assigned straight from
...
//
// THE GARDEN CARES ABOUT THE CORNER MORE THAN THE CAVES DO
//
// A garden is hedges and flowerbeds with gaps between them, so it is nearly all
// doorways -- and a body is 192 subcells across in a 256-subcell cell, which
// leaves 32 subcells of slack the player cannot see. Every one of those gaps
// was a coin toss. See src/core/steer.ts.
//
// Everything else is calm/3 exactly: the wand still freezes the pond and the
// whole pond at once, the bear still hunts and the bunnies still do not, and
// the same fires go out for the same bucket.
//
...
export const CALM_V4_BEHAVIOUR = 4;
...
export class CalmV4 implements Engine {
  readonly id = "calm" as const;
  readonly behaviourVersion = CALM_V4_BEHAVIOUR;
