// Calm, behaviour version 2. The garden, with something in it.
//
// calm/1 was a PLACE: no exit, no win, no loss, no clock, no weapon. It was
// asked for that way and adr/0040 wrote down why. This is the other thing that
// was asked for:
//
//   "In the garden game the bunny, bird and squirrel should just wonder not
//    attack or follow, and no weapons should be used, and the garden needs an
//    exit"
...
// A LEVEL. So calm/2 is roam/8's rules -- collect the flowers, reach the gate,
// hearts, a clock, a weapon -- wearing the garden's.
//
// WHAT IS NEW HERE, AND IT IS NEW TO THE WHOLE PROJECT
//
// Which creature it is now decides what it DOES. Everywhere else the three
// enemies "walk, chase and die exactly alike -- what changes is what a child
// sees walking towards them", and level.guardArt has only ever chosen a
// drawing. In this garden:
//
//   G  the bear      hunts you, hurts you, and can be fought
//   B  the bunny     wanders, and is not part of the fight at all
//   D  the squirrel  the same
//
// A bunny you can kill with a sword is not a bunny. A bunny that chases you is
// not one either. There is no way to have both a thing that hunts and things
// that do not while all three are the same entity kind -- the kind field is
// three bits with one value left, and spending it was refused for this.
//
// So the art field is load-bearing HERE and nowhere else. It is level data, it
// is fixed for the life of a level, and it travels in the link, so a run still
// replays identically for everybody -- which is the property that actually
// matters. The old sentence was a design choice, not a rule, and calm/2 is a
// new build, which is the only place a choice like that may change.
//
// Derived from roam/8 rather than from calm/1, because what it needed back was
// everything calm/1 took out.
//
// The garden's own rules, on top of that:
//
//   a POND is solid            you walk round one, or over a bridge. Everywhere
//                              else the same entity is a flame you may walk
//                              through for a heart; here it is water.
//   a BRIDGE is a ladder       the tile a top-down world never had a use for
//   the bucket is gone         nothing in a garden is on fire
//
// roam/8's water-bucket prose lived here and has gone with it: there is no
// fire to put out, so POUR_TICKS and the doused list simply never fire.
...
export const CALM_V2_BEHAVIOUR = 2;
...
export class CalmV2 implements Engine {
  readonly id = "calm" as const;
  readonly behaviourVersion = CALM_V2_BEHAVIOUR;
  ...
  private fits(x: number, y: number): boolean {
    ...
    // that makes it solid. See adr/0040.
    {
      ...
      if (isFire(this.level, pondX, pondY) && !isLadder(this.level, pondX, pondY)) return false;
    }
