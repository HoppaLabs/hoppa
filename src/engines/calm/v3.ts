// calm/3: the garden, where a wand freezes the pond.
//
// A copy of calm/2, not an edit of it (hard rule 3): every garden link anybody
// has sent pins calm/2 and has to keep replaying under exactly the rules it was
// beaten under. What is added here is one verb.
//
// WHY IT NEEDED A NEW BUILD AND THE BUCKET DID NOT
//
// The bucket was already in calm/2 -- every top-down engine copied roam/8's
// douse -- and the fix on day 21 was that nobody could SEE the button, which is
// presentation. This is not: a wand that freezes water changes which cells you
// can stand on, which changes hearts, which changes whether a log wins. The ice
// is in stateHash() and so it is a new number on the wire.
//
// WHY THE WAND
//
// "Use the wand to freeze water." The wand already freezes enemies, and its
// whole character is making a dangerous thing safe FOR A WHILE rather than
// removing it. A pond is a dangerous thing. So the wave does not get a second
// meaning: it gets a second target, and the duration is the same one the
// enemies use so there is only one thing for a child to learn.
//
// A sword gets nothing, and that is the trade rather than an omission -- the
// same shape as "a wand never kills". iceTicksFor() is 0 for a sword and the
// whole mechanism is inert.
//
// WHY THE WHOLE POND
//
// A cell at a time would mean standing in the water to reach the next one,
// paying a heart per square. That is not a way across; it is a slower way of
// drowning. See freezeWater().
//
...
export const CALM_V3_BEHAVIOUR = 3;
...
export class CalmV3 implements Engine {
  readonly id = "calm" as const;
  readonly behaviourVersion = CALM_V3_BEHAVIOUR;
