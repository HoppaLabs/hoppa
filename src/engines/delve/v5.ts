// Delve, behaviour version 5.
//
// The capabilities a creature is read for, and what they do here:
//
//   * HASTE -- SPEED. An occasional free step: you move, the world does not.
//   * FORCE -- STRENGTH. Barge a guard: it reels, and stands stunned while you
//              walk past. Below the threshold, walking into a guard still ends
//              the run.
//   * GUARD -- how many spottings you survive before the alarm is fatal.
//   * REACH -- a long arm lifts gems from the next cell.
//
// MASS IS NO LONGER READ. See docs/adr/0008. In v4 it set the noise radius, so
// a heavy creature was simply worse at Delve and there was nothing it was
// better at; and with every sprite the same 16x16, "weight" was a number with
// nothing behind it. Speed and strength mean the same thing in any engine, so
// they are what a creature carries from one to the next.
//
// The noise radius is now the same for everybody.
//
// v4 still reads MASS and always will: links made under it must keep playing
// identically. The axis stays in the data; nothing from here on looks at it.
//
// Guards are no longer a pure function of the turn counter, because barging one
// knocks it off its cycle. Each guard now carries its own phase and a stun
// countdown, and both are hashed -- that is the price of strength meaning
// something, and it is paid in authoritative state, not in a PRNG.
//
// NEVER edit this file's behaviour in place. Add v6.ts.
...
export const DELVE_V5_BEHAVIOUR = 5;
 ...
 * block it, which is far easier for a kid to predict than a line-of-sight cone.
 *
 * The same for every creature now. It used to scale with MASS; see the note at
 * the top of this file and docs/adr/0008.
 */
...
/**
 * STRENGTH. A creature at or over the line can barge a guard instead of being
 * caught by it: the guard reels back and stands there, stunned, for this many
 * turns. 0 means it cannot barge at all.
 *
 * This is the same idea FORCE will carry into Shove, where it pushes blocks --
 * strength means one thing everywhere, which is the point of the change.
 */
export function bargeStunFor(creature: Creature): number {
  if (creature.caps.FORCE >= 192) return 4;
  if (creature.caps.FORCE >= 128) return 3;
  return 0;
}
...
export class DelveV5 implements Engine {
  readonly id = "delve" as const;
  readonly behaviourVersion = DELVE_V5_BEHAVIOUR;
  ...
  private readonly bargeStun: number;
  /** Each guard's own place in its cycle, and how long it is still reeling. */
  private readonly phase: Int32Array;
  private readonly stun: Int32Array;
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(
        `level has ${level.treasureCells.length} treasures; delve v5 can track ${MAX_TREASURE} ` +
    ...
    this.bargeStun = bargeStunFor(creature);
    this.haste = 0;
    this.patrols = patrolsFor(level);
    this.phase = new Int32Array(this.patrols.length);
    this.stun = new Int32Array(this.patrols.length);
  ...
  step(input: number): Status {
    ...
    const fromX = this.x | 0;
    const fromY = this.y | 0;
    ...
    // You walked into a cell a guard was standing in. With enough STRENGTH that
    // is a barge rather than a death: the guard reels, and you hold your ground
    // -- you shoved it, you did not swap with it.
    const met = this.guardOn(idx(this.x, this.y));
    if (met >= 0) {
      if (this.bargeStun > 0) {
        this.stun[met] = this.bargeStun | 0;
        this.barged = true;
        this.x = fromX;
        this.y = fromY;
      } else {
    ...
    // A free step is the same instant as the step before it: the world does not
    // get another go at you.
    if (!this.freeStep) this.moveGuards();

    // ...and a guard that walked onto you has you, however strong you are.
    if (this.guardOn(idx(this.x, this.y)) >= 0) return this.capture();
  ...
  /** Where guard `i` is standing right now. */
  private guardCell(i: number): number {
    return patrolCellAt(this.patrols[i] as Patrol, this.phase[i] as number);
  }

  /** The guard standing on this cell, or -1. */
  private guardOn(cell: number): number {
    for (let i = 0; i < this.patrols.length; i = (i + 1) | 0) {
      if (this.guardCell(i) === cell) return i;
    }
    return -1;
  }

  /** Advance every guard that is not still reeling. */
  private moveGuards(): void {
    for (let i = 0; i < this.patrols.length; i = (i + 1) | 0) {
      if ((this.stun[i] as number) > 0) {
        this.stun[i] = ((this.stun[i] as number) - 1) | 0;
      } else {
        this.phase[i] = ((this.phase[i] as number) + 1) | 0;
      }
    }
  }
  ...
  render(): Uint8Array {
    ...
    for (let i = 0; i < this.patrols.length; i = (i + 1) | 0) {
      this.tiles[this.guardCell(i)] = (this.stun[i] as number) > 0 ? TILE_GUARD_REELING : TILE_GUARD;
    }
  ...
  stateHash(): number {
    ...
    // Guards can be knocked off their cycle now, so where each one is and how
    // long it is still reeling are authoritative state, not derivable from the
    // turn counter the way they were up to v4.
    for (let i = 0; i < this.patrols.length; i = (i + 1) | 0) {
      h = hashInt32(h, this.phase[i] as number);
      h = hashInt32(h, this.stun[i] as number);
    }
    return h | 0;
  }
  ...
  /** True when the last step barged a guard aside. Presentation only. */
  didBarge(): boolean {
    return this.barged;
  }

  /** How hard this creature shoves; 0 means it cannot. */
  bargePower(): number {
    return this.bargeStun;
  }
