// Dash, behaviour version 4. Side-on, real time.
//
// v4 changes one thing: you pick treasure up by going to it, rather than by
// being within a sword's length of it -- the same change roam/5 made, and the
// same reason. Up to v3 the pickup borrowed the weapon's reach, so a gem could
// be collected from the platform below without ever climbing to it.
//
// Everything else is v3 exactly. See docs/adr/0029.
...
export const DASH_V4_BEHAVIOUR = 4;
...
export class DashV4 implements Engine {
  readonly id = "dash" as const;
  readonly behaviourVersion = DASH_V4_BEHAVIOUR;
