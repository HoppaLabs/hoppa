// Dash, behaviour version 10. From the side, real time.
//
// v10 puts boxes in the room, and in a side-on game there is only one right way
// to open one: you jump into it from underneath.
//
//     "What about if we had question mark boxes like in Mario?"
//     "I want the author to decide if it's treasure or an enemy."
//
// A box is a WALL until it is opened -- it is a platform you can stand on and a
// ceiling that stops your jump -- and the thing that stops your jump is exactly
// the thing you open by bumping it. That is the whole mechanic, and it is not
// anybody's property: hitting a block from below is how a platformer has worked
// since the mid eighties, described in public a hundred times over. The
// characters, the tiles and the rooms here are ours.
//
// A swing opens one too, because the alternative is a box on a ledge you cannot
// get under, and a room that cannot be finished is worse than a room that is
// easy. The bump is the good way; the swing is the way that always works.
//
// WHAT COMES OUT IS THE AUTHOR'S CHOICE, AND IT IS ON THE WIRE
//
// The first design derived it from the level's seed, which was free and wrong:
// a surprise the author cannot aim is a lottery, not a level. One bit per box.
//
// BOTH KINDS LOOK THE SAME IN PLAY. The engine emits TILE_BOX either way. A
// trap your friend can see coming is not a trap; the editor draws them apart
// because the author is the one person entitled to know.
//
// dash/9 stays and stays exact. Every link ever sent still replays.
...
export const DASH_V10_BEHAVIOUR = 10;
...
interface Walker {
   ...
   *
   * Built at construction like every other walker and simply not let out yet:
...
export class DashV10 implements Engine {
  readonly id = "dash" as const;
  readonly behaviourVersion = DASH_V10_BEHAVIOUR;
  readonly consumes = CONSUMES;

  private readonly level: Level;
  private readonly creature: Creature;
  private readonly walkers: Walker[];
  /** 1 where a box has been opened. */
  private readonly opened: Uint8Array;
  /** Treasure slot for the gem in the box at this cell, or -1. */
  private readonly boxSlot: Int8Array;
  /** Index into `walkers` of what is inside the box at this cell, or -1. */
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    ...
    this.boxWalker = new Int16Array(GRID_W * GRID_H).fill(-1);
    ...
    for (let i = 0; i < level.boxCells.length; i = (i + 1) | 0) {
      ...
      if ((level.boxHolds[i] ?? 0) === BOX_HOLDS_ENEMY) {
        this.boxWalker[cell] = this.walkers.length | 0;
        this.walkers.push({
          x: cellCentre(cell % GRID_W),
          y: (cellCentre((cell / GRID_W) | 0) + REST) | 0,
          dir: 1,
    ...
    if (slot > MAX_TREASURE) {
      throw new Error(`level has ${slot} gems counting boxes; dash v10 holds ${MAX_TREASURE}`);
    }
  ...
  private moveVertically(buttons: number): void {
    ...
    while (left !== 0) {
      ...
      if (this.fits(this.x, ny)) {
        ...
        // THE HEAD-BUMP. Going UP and something stopped you: if the thing that
        // stopped you is a box, you have just opened it.
        //
        // The cells checked are the ones a BODY would touch, both sides of it,
        // because a creature is three quarters of a cell wide and jumping up a
        // one-cell gap between two boxes should open one of them rather than
        // neither. Left first, so two boxes at once always resolve the same
        // way for everybody replaying the same log.
        if (slice < 0) {
          const head = (toCell((this.y - BODY) | 0)) | 0;
          const west = toCell((this.x - BODY + 1) | 0);
          const east = toCell((this.x + BODY - 1) | 0);
          if (!this.openBoxAt(west, head)) this.openBoxAt(east, head);
        }
        this.vy = 0;
        break;
      }
   ...
   * it is opened and there must be exactly one place that knows it. It is a
   * platform and a ceiling until then.
   */
  ...
  /**
   * Open the box at this cell, if there is one there and it is still shut.
   *
   * Returns whether anything happened, because the head-bump wants to know: a
   * bump that opened something is a bump worth stopping the jump for, and a
   * bump against ordinary wall is just a ceiling.
   */
  private openBoxAt(cx: number, cy: number): boolean {
    if (cx < 0 || cx >= GRID_W || cy < 0 || cy >= GRID_H) return false;
    const cell = idx(cx, cy);
    if (this.opened[cell] === 1) return false;
    const slot = this.boxSlot[cell] as number;
    const seat = this.boxWalker[cell] as number;
    if (slot < 0 && seat < 0) return false;
    ...
    if (slot >= 0) {
      ...
      // read as one you had missed.
      this.collected = (this.collected | (1 << slot)) | 0;
    }
    if (seat >= 0) (this.walkers[seat] as Walker).hidden = 0;
    return true;
  }
  ...
  private strike(): void {
    ...
    // ...and last, the box you are facing.
    //
    // LAST, because opening it first would put a freshly released walker
    // inside the same swing's reach: one press would open the box AND kill
    // what came out, which is a button rather than a gamble. Measured in
    // roam/10, where it arrived already down.
    const at = (toCell(this.x) + dx) | 0;
    this.openBoxAt(at, toCell(this.y));
  }
  ...
  render(): Uint8Array {
    ...
    // The boxes, over the wall they stand in as. ONE tile for both kinds; an
    // opened one stays put, because a box that vanished would leave a hole in
    // the wall the author never drew -- and in a side-on room that hole is a
    // platform somebody was standing on.
  ...
  enemiesLeft(): number {
    let n = 0;
    for (let i = 0; i < this.walkers.length; i = (i + 1) | 0) {
      const one = this.walkers[i] as Walker;
