...
export const DASH_V2_BEHAVIOUR = 2;
...
export class DashV2 implements Engine {
  readonly id = "dash" as const;
  readonly behaviourVersion = DASH_V2_BEHAVIOUR;
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(`level has ${level.treasureCells.length} treasures; dash v2 holds ${MAX_TREASURE}`);
    }
    this.level = level;
