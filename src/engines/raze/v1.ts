// raze/1 -- the adventure game, with buildings that come down.
//
// Asked for with the city: "the buildings should be destructible and turn into
// fire when attacked by a jaeger or kaiju". Everything about that is roam/8
// except the last part, so this file is roam/8 with about forty lines added
// and one rule changed.
//
// WHY IT IS A NEW ENGINE ID AND NOT A NEW ROAM VERSION
//
// It could have been roam/9. It is not, for two reasons that pull the same
// way:
//
//   * Hard rule 3 says never change engine behaviour in place. roam/9 would
//     have kept that rule -- old links pin roam/8 -- but it would have made
//     every NEW adventure level a level with smashable walls, which is a large
//     design change to a game nobody asked to change.
//   * Hard rule 4 says cosmetics never touch stateHash(). So the CITY SKIN
//     cannot decide the rules: a tileset id is presentation, and presentation
//     may not reach the engine. The rules have to live somewhere the wire
//     format already carries as rules, and the engine id is that place.
//
// ENGINE_IDS is four bits and append-only, with ten of sixteen slots free. One
// of them is now "raze". A city level says raze/1 and a cave level says
// roam/8, and neither can turn into the other by being redrawn.
//
// WHAT IS ACTUALLY DIFFERENT
//
//   * A swing smashes the wall directly ahead of you, IF the creature is
//     strong enough (FORCE 4 of 5). This is the first thing in the game that
//     strength unlocks rather than merely scales.
//   * A smashed wall burns for three seconds and then becomes floor. Burning
//     costs a heart the way any fire does. Permanent fire would let a child
//     wall themselves into a corner with their own weapon; a timer makes a
//     smashed building a shortcut you have to wait for.
//   * Only the PLAYER smashes anything. A kaiju that breaks walls can seal a
//     player behind a fire they did not light, and the fantasy is that the
//     jaeger is the one wrecking the city anyway. If the monsters should do it
//     too, that is raze/2 and a fresh set of measurements.
//
// WHAT IS DELIBERATELY NOT DIFFERENT
//
//   * Patrols are worked out once, from the walls the level was DRAWN with.
//     Smashing a corridor open does not re-route the thing pacing it. That is
//     a choice: a patrol is a property of the room a child drew, and a guard
//     whose beat changes under it is a guard nobody can plan around.
//   * A razed wall only ever ADDS somewhere to walk, so a level that passed
//     L3 and L4 still passes them, and a room the bot could finish it can
//     still finish. Nothing in the verifier needed changing.
...
export const RAZE_V1_BEHAVIOUR = 1;
...
export class RazeV1 implements Engine {
  readonly id = "raze" as const;
  readonly behaviourVersion = RAZE_V1_BEHAVIOUR;
