// raze/3: the city, with boxes in it.
//
// A copy of raze/2, not an edit of it (hard rule 3): every city link anybody
// has sent pins raze/2 and has to keep replaying under exactly the rules it was
// beaten under.
//
//     "And then add surprise boxes to all 18 shipped games"
//
// Seventeen of the eighteen were a level edit -- roam, dash, calm and swim have
// had boxes since day 22. The city is the eighteenth, and it needed an engine.
//
// A CRATE IS NOT A BUILDING
//
// This is the one thing raze has to decide that no other engine does. A box is
// a WALL, and in this game walls come down: left alone, the jaeger would raze a
// crate into burning rubble, which reads as the box being destroyed and its
// contents lost. So smash() skips boxes and the same swing opens them instead.
// One press still does both jobs, and "hit the thing in front of you" still
// means one thing rather than two.
//
// The order is strike, then smash, then open -- opening LAST, for the reason
// roam/10 found out the hard way: open first and a monster that has just come
// out is standing inside the same swing's reach, so the press that let it out
// also kills it.
//
// Everything else is raze/2 exactly.
//
...
import {
  TILE_ACTOR, TILE_BOX, TILE_BOX_OPEN, TILE_EXIT_LOCKED, TILE_EXIT_OPEN,
  TILE_FIRE, TILE_FLOOR, TILE_GUARD, TILE_GUARD_REELING, TILE_TREASURE,
  TILE_WALL,
} from "../../core/tiles.ts";
import {
...
export const RAZE_V3_BEHAVIOUR = 3;
...
export class RazeV3 implements Engine {
  readonly id = "raze" as const;
  readonly behaviourVersion = RAZE_V3_BEHAVIOUR;
  ...
  /** 1 where a box has been opened. Parallel to `razed`, and read the same
   *  way: an opened box is no longer a wall. */
  private readonly opened: Uint8Array;
  /** Which treasure slot a box holds, or -1. */
  private readonly boxSlot: Int8Array;
  /** Which enemy is waiting inside a box, or -1. */
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(`level has ${level.treasureCells.length} treasures; raze v3 holds ${MAX_TREASURE}`);
    }
    this.level = level;
    ...
    this.opened = new Uint8Array(GRID_W * GRID_H);
    this.boxSlot = new Int8Array(GRID_W * GRID_H).fill(-1);
    this.boxEnemy = new Int16Array(GRID_W * GRID_H).fill(-1);
    ...
    // The boxes, after the patrolling enemies so that a level's own guards
    // keep the indices they have always had.
    //
    // A gem in a box counts toward the door: its slot comes after the level's
    // own gems in the same mask, so a room may hold eight between them. That
    // is what stops boxes being decoration -- a room can put a gem in one and
    // mean it, and the friend has to open the thing to get out.
    let slot = level.treasureCells.length | 0;
    for (let i = 0; i < level.boxCells.length; i = (i + 1) | 0) {
      const cell = level.boxCells[i] as number;
      if ((level.boxHolds[i] ?? 0) === BOX_HOLDS_ENEMY) {
        this.boxEnemy[cell] = this.enemies.length | 0;
        this.enemies.push({
        ...
        this.boxSlot[cell] = slot as number;
        slot = (slot + 1) | 0;
      }
    }
    if (slot > MAX_TREASURE) {
      throw new Error(`level has ${slot} gems counting boxes; raze v3 holds ${MAX_TREASURE}`);
    }
  ...
  step(held: number): Status {
    ...
    if (this.swing === 0 && (actDown || this.swingBuffer > 0)) {
      ...
      this.smash();
      // ...and the same swing opens a crate. Last, so a monster that has just
      // come out is not standing inside the reach of the press that let it
      // out.
      this.openBoxAhead();
    }
  ...
  private wallAt(cx: number, cy: number): boolean {
    ...
    // Two ways a wall stops being one here: the jaeger brought it down, or it
    // was a box and somebody opened it. Both are one-way, which is what makes
    // them safe -- the world only ever changes toward more room.
    const cell = idx(cx, cy);
    return this.razed[cell] === 0 && this.opened[cell] === 0;
  }
  ...
  private moveEnemies(): void {
    for (let i = 0; i < this.enemies.length; i = (i + 1) | 0) {
      ...
      // Still in its box: it exists, it is standing on the box, and it does
      // nothing at all until the box is opened.
      if (enemy.hidden !== 0) continue;
      // ...and a boxed enemy has no patrol, because the boxes are appended
      // after the level's patrolling guards. Without this the cast list and
      // the patrol list stop lining up and the next line reads undefined.
  ...
  private smash(): void {
    ...
    // A CRATE IS NOT A BUILDING. Left to itself this would raze a box into
    // burning rubble, which reads as the box being destroyed and whatever was
    // in it lost. The same swing opens it instead -- see openBoxAhead().
    if ((this.boxSlot[cell] as number) >= 0 || (this.boxEnemy[cell] as number) >= 0) return;
    this.razed[cell] = 1;
  ...
  /**
   * Open the box directly ahead, if there is one.
   *
   * The same cell smash() and the bucket pick: the one ahead of the actor's
   * CENTRE. Called LAST in the swing, after strike() and smash(), because
   * opening first puts a monster that has just come out inside the same
   * swing's reach -- roam/10 found that out by killing one on the press that
   * let it out.
   */
  private openBoxAhead(): void {
    const ax = (toCell(this.x) + (FACE_DX[this.facing] as number)) | 0;
    const ay = (toCell(this.y) + (FACE_DY[this.facing] as number)) | 0;
    if (ax < 0 || ax >= GRID_W || ay < 0 || ay >= GRID_H) return;
    const cell = idx(ax, ay);
    if (this.opened[cell] === 1) return;
    const slot = this.boxSlot[cell] as number;
    const seat = this.boxEnemy[cell] as number;
    if (slot < 0 && seat < 0) return;
  ...
  private strike(): void {
    ...
    for (let i = 0; i < this.enemies.length; i = (i + 1) | 0) {
      const enemy = this.enemies[i] as Enemy;
      if (enemy.down !== 0) continue;
      // Still in its box, and therefore not hittable.
      //
      // roam/10 avoided this by opening the box LAST in the swing, and raze
      // does the same -- but that is not enough here, because a boxed monster
      // stands ON its box, which is exactly the cell the swing reaches. So on
      // the tick you knock the crate open, strike() has already run over a
      // monster that is technically in range and technically alive, and it
      // died in the box. The test read "enemies 0" after opening one.
      if (enemy.hidden !== 0) continue;
  ...
  render(): Uint8Array {
    ...
    // BOTH KINDS DRAW THE SAME. A trap your friend can see coming is not a
    // trap, so the engine emits TILE_BOX whichever is inside. The editor tells
    // them apart because the author is the one person entitled to know, and
    // that is a fact about the screen rather than about the game -- which is
    // exactly the line hard rule 5 draws.
    const boxes = this.level.boxCells;
    for (let i = 0; i < boxes.length; i = (i + 1) | 0) {
      const cell = boxes[i] as number;
      this.tiles[cell] = this.opened[cell] === 1 ? TILE_BOX_OPEN : TILE_BOX;
    }
  ...
  stateHash(): number {
    ...
    // ...and which boxes are open, for the same reason. Whether a crate has
    // been hit changes what is a wall and what enemies are in the room, so two
    // runs of one log have to agree about it.
  ...
  enemiesLeft(): number {
    let n = 0;
    for (let i = 0; i < this.enemies.length; i = (i + 1) | 0) {
      const one = this.enemies[i] as Enemy;
      if (one.down === 0 && one.hidden === 0) n = (n + 1) | 0;
    }
    return n;
  }
  ...
  /** Was a box opened this tick, where, and did it give a gem? The page draws
   *  the pop off these; the engine never says what a pop looks like. */
  ...
  /** How many boxes are still shut. */
  boxesLeft(): number {
    let n = 0;
    const boxes = this.level.boxCells;
    for (let i = 0; i < boxes.length; i = (i + 1) | 0) {
      if (this.opened[boxes[i] as number] === 0) n = (n + 1) | 0;
    }
    return n;
  }
