// Swim, behaviour version 2. Underwater, from the side, real time.
//
// v2 is the version where the water goes somewhere.
//
// WHY THIS IS THE POINT OF THE ENGINE
//
// v1 could swim and could drown, and a strong creature had no reason to exist
// in it. That is the flaw currents fix, and it is worth being precise about it,
// because it is not a matter of taste.
//
// The premise of the whole game is that your friend plays your level with THEIR
// creature, who is good at different things. In the side-on game STRENGTH buys
// jump height, so a strong creature reaches a ledge a fast one cannot: the two
// builds take different routes through the same room. Underwater, with free
// movement, nothing is out of reach -- so strength bought hearts and sword hits
// and nothing about ROUTE at all, and half the character system was dead in
// water.
//
// A current gives it back, and inverts the obvious:
//
//   creature   speed   push against them   net, swimming into it
//   Nim         50           46                     +4     crawls
//   Pell        38           30                     +8
//   Bash        26           14                    +12     fastest of the three
//
// The slowest creature in the game is the fastest one THROUGH a current, and
// the fastest is nearly stopped by it. So Nim goes the long way round and gets
// there first; Bash goes straight through. Same room, two routes, which is the
// thing the game is for.
//
// HOW THEY ARE PUT IN A LEVEL
//
// Per cell, painted by dragging -- the direction is the direction your finger
// went. Not one arrow that pushes a whole corridor: that would be cheaper on
// the wire and it would be a rule a child cannot see the edges of. What you
// paint is what flows.
//
// It is a DRIFT, not a shove: the push moves you, it does not go into your
// momentum. Momentum that accumulated would let a current fling you across the
// room at a speed nothing in the level could have predicted.
//
// Everything else is v1 exactly.
...
import {
  TILE_ACTOR, TILE_EXIT_LOCKED, TILE_EXIT_OPEN, TILE_FIRE, TILE_FLOOR, TILE_FLOW,
...
export const SWIM_V2_BEHAVIOUR = 2;
...
export class SwimV2 implements Engine {
  readonly id = "swim" as const;
  readonly behaviourVersion = SWIM_V2_BEHAVIOUR;
