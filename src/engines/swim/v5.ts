...
//
// A copy of swim/4, not an edit of it (hard rule 3): every reef link anybody
// has sent pins swim/4 or earlier and has to keep replaying under exactly the
// rules it was beaten under.
//
// THE REEF WAS ALREADY MOSTLY RIGHT
//
// "It feels like moving a cursor" was about the OTHER four builds. Swimming has
// had momentum since swim/1 -- holding builds speed towards a cap and letting
// go leaves you to the water -- and that gap between ACCEL and DRAG is the best
// bit of feel in the game. It is not touched here, and neither are the
// currents.
//
// WHAT WAS STILL WRONG
//
// Three things swim shared with the rest, all of them invisible until measured:
//
//   * A diagonal was 41% faster than any straight line, because the cap was
//     applied per axis. Every player finds that within a minute and then never
//     swims straight again.
//   * A gap in the rock you were nearly lined up with stopped you dead. A body
//     is 192 subcells across in a 256-subcell cell, so a one-cell gap has 32
//     subcells of slack that nothing on screen shows you.
//   * A press that landed during a swing was dropped on the floor, and being
//     hit teleported you two whole cells in a single tick.
//
// See src/core/steer.ts and docs/adr/0059.
//
// Everything else is swim/4 exactly: the same wand freezes the same urchins,
// the currents push exactly as hard, and nothing falls.
//
...
export const SWIM_V5_BEHAVIOUR = 5;
...
export class SwimV5 implements Engine {
  readonly id = "swim" as const;
  readonly behaviourVersion = SWIM_V5_BEHAVIOUR;
