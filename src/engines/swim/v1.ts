// Swim, behaviour version 1. Underwater, from the side, real time.
//
// The third game. Asked for as "we need an underwater level", then "maybe we
// could try a whole other style other than from above, from the side?", and
// finally settled with a reference: "swim and currents, like Echo the Dolphin".
//
// WHY IT IS AN ENGINE AND NOT A TILESET
//
// Free movement in four directions is what the top-down game already does, and
// swimming is the same contract -- so the movement below starts as roam/8's and
// is honest about it. What is NOT shared is everything round it: this level is
// drawn from the side, its frame is rock with an open surface on top rather
// than a box, its paint rules are different, and it has a thing that kills you
// which has nothing to do with enemies.
//
// It is emphatically not a dash variant. Gravity-every-tick is the rule that
// makes side-on levels mean anything -- platforms, reachable heights, ladders,
// the whole route model the bot uses. Let a player rise freely and all of it
// dissolves at once.
//
// AIR IS UP
//
// The one mechanic worth stealing from Ecco, and the reason it is safe to
// steal. A breath meter is usually cruel: it punishes the child who explores
// slowly, and it hides its own answer in air pockets somebody has to find.
// Ecco's does neither, because the answer is a DIRECTION. The surface is the
// top of the screen, it is always there, and "swim up to breathe" is a rule a
// six-year-old has understood before anybody explains it.
//
// It also gives the level's shape a question to ask, which is the whole job of
// a hazard here: a room whose top is sealed with rock is tense, and a child
// authors that tension by painting a line.
//
// MOMENTUM
//
// Water does not let go of you and it does not stop you dead. Holding a
// direction builds speed; letting go coasts. That is the other half of what
// makes swimming feel unlike walking, and it costs two integers.
//
// WHAT IS NOT HERE YET
//
// Currents. They are the reason this engine earns its keep: with free movement
// and nothing pushing you, STRENGTH has no routing job underwater, which would
// leave half the character system dead in water. They need a swim-only field in
// the codec, so they get their own version.
...
export const SWIM_V1_BEHAVIOUR = 1;
...
/**
 * How long one breath lasts, in ticks. Thirty ticks to the second, so twenty
 * seconds.
 *
 * Long enough to cross the level and look at something on the way; short enough
 * that where the surface is stays on your mind. It is the number most likely to
 * be wrong, and it is one number, so it is easy to move.
 */
export const AIR_TICKS = 600;

/** Ticks between hearts once the air has run out. */
export const DROWN_TICKS = 45;

/**
 * The row you can breathe in.
 *
 * The top row of the grid, which the swim frame leaves open where every other
 * frame puts rock. That is the whole of the rule: air is a place, the place is
 * the top of the screen, and it is drawn as the surface of the water.
 */
export const SURFACE_ROW = 0;

/**
 * How fast a breath comes back. Six times faster than it goes, so a gulp is a
 * moment at the surface and not a wait.
 */
...
export class SwimV1 implements Engine {
  readonly id = "swim" as const;
  readonly behaviourVersion = SWIM_V1_BEHAVIOUR;
  ...
  private air: number;
  /** Counts down between hearts once the air is gone. */
  private drowning: number;
   ...
   *
   * The surface is the top row and nowhere else. Being there fills you up;
   * being anywhere else empties you. When it is empty you lose a heart every
   * DROWN_TICKS -- steadily, with a warning, rather than all at once, because
   * the first time a child runs out they need to understand what happened in
   * time to do something about it.
   */
  private breathe(): void {
    if (toCell(this.y) <= SURFACE_ROW) {
      this.air = Math.min(AIR_TICKS, (this.air + BREATH_IN) | 0);
      this.drowning = 0;
      return;
    }
    ...
    this.drowning = (this.drowning + 1) | 0;
    if (this.drowning < DROWN_TICKS) return;
    this.drowning = 0;
    this.hp = (this.hp - 1) | 0;
  ...
  stateHash(): number {
    ...
    h = hashInt32(h, this.air);
    h = hashInt32(h, this.drowning);
  ...
  /** Breath left and breath total, for the meter. Presentation only. */
  breath(): { left: number; full: number } {
    return { left: this.air | 0, full: AIR_TICKS };
  }
  /** True on the tick a pour actually put something out. Presentation only. */
