// Swim, behaviour version 3. Underwater, from the side, real time.
//
// v3 is v2 with the AIR TAKEN OUT. You do not drown; you just swim.
//
// WHY IT WENT
//
// Asked for directly -- "I think we should remove the need for air in the
// underwater levels" -- the day after this was reported from a phone: "the
// player character seems to randomly get hurt after passing a bubble, very
// odd?" It was not the bubble. It was the air running out at AIR_TICKS and a
// heart going every DROWN_TICKS, announced as "That hurt." on a tick with
// nothing anywhere near.
//
// The page was made to say so (see src/web/play/breath.ts) and the mechanic
// still did not earn its place. Air is a CLOCK, and a clock is the one thing
// that turns exploring a room into being chased out of it. Nothing else in
// this game takes a heart for taking your time.
//
// What survives is what made the water worth building: momentum, and currents.
// Currents are what give STRENGTH a job underwater (adr/0039) and they are
// untouched, so the two-routes-through-one-room property this engine exists
// for is exactly as it was.
//
// v2 IS STILL SHIPPED AND STILL DROWNS YOU. Hard rule 3: every reef link
// already sent pins swim/2 and plays exactly as it did. This is a new build
...
export const SWIM_V3_BEHAVIOUR = 3;
...
export class SwimV3 implements Engine {
  readonly id = "swim" as const;
  readonly behaviourVersion = SWIM_V3_BEHAVIOUR;
