...
export const ROAM_V3_BEHAVIOUR = 3;
...
export class RoamV3 implements Engine {
  readonly id = "roam" as const;
  readonly behaviourVersion = ROAM_V3_BEHAVIOUR;
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(`level has ${level.treasureCells.length} treasures; roam v3 holds ${MAX_TREASURE}`);
    }
    this.level = level;
