// Roam, behaviour version 7. From above, real time.
//
// v7 changes one thing: a chasing enemy moves one direction at a time, the way
// a thumb on a d-pad does.
//
// Up to v6 a chase applied stepX AND stepY in the same tick, so an enemy cut
// diagonals toward you at a speed no player can match -- moving on the
// diagonal covers about 1.41 times the ground of an axis step, for free, and
// only the enemy ever did it. A player CAN hold two directions at once, but on
// a phone that means two thumbs on a five-button pad, so in practice never.
// Reported as "the enemy should only move the way the player character can
// move, one direction at a time".
//
// The dominant axis wins; if that step is blocked, the other is tried, so an
// enemy still slides along a wall instead of pressing into it. A tie goes to
// the horizontal, arbitrarily but always, because a replay on somebody else's
// phone a week later has to make the same choice.
//
// Everything else is v6 exactly, fire included.
...
export const ROAM_V7_BEHAVIOUR = 7;
...
export class RoamV7 implements Engine {
  readonly id = "roam" as const;
  readonly behaviourVersion = ROAM_V7_BEHAVIOUR;
