// Roam, behaviour version 6. From above, real time.
//
// v6 adds fire: a hazard that does not move.
//
// Every danger before this walked. A guard is a TIMING problem -- wait, dodge,
// or hit it. Fire is a ROUTE problem, which is a different question and the one
// a level's SHAPE can pose. It also has no patrol, so spec L5 has nothing to
// say about it, and an open room becomes designable again.
//
// It costs a heart on contact, with the same mercy window a guard's hit gives,
// and nothing puts it out: the weapon already answers guards, and giving it a
// second use would make both weaker. See docs/adr/0034.
//
...
export const ROAM_V6_BEHAVIOUR = 6;
...
export class RoamV6 implements Engine {
  readonly id = "roam" as const;
  readonly behaviourVersion = ROAM_V6_BEHAVIOUR;
