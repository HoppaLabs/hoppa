// Roam, behaviour version 4. Top-down, real time.
//
// v4 slows the enemies down, because v3's were faster than half the creatures
// you could build. Two things fell out of that, and both were reported from
// real play as "I die too quickly and they move too fast":
//
//   * A creature that spent its points on STRENGTH walked at 26 against an
//     enemy at 30. It could never break away -- strength bought hearts and
//     took away any means of using them. The same shape of trap as dash/1's
//     creature that could not climb: a legitimate build the game punished.
//
//   * Knockback bought nothing. You are thrown two cells; at 30 an enemy
//     covered that in 0.6 of a second, well inside the 1.7 seconds of mercy,
//     so it was standing on you the instant mercy ended. Hearts came off in a
//     chain you could not break.
//
// At 22 every build outruns an enemy -- the slowest by 18%, the fastest by
// 127% -- so speed still decides HOW easily you get away, rather than whether
// you can at all. See docs/adr/0021.
//
// v1, v2 and v3 are untouched and still shipped.
//
// (v3's own header follows.)
//
// Roam, behaviour version 3. Top-down, real time.
//
// v3 gives the weapon a job. In v2 a wand was a costume: it reached as far as a
// sword and hit as hard, and the only difference was the picture. Here they are
// two different answers to the same problem:
//
//   * a SWORD kills. It takes between one and four swings depending on
//     strength, and between them the enemy is only briefly down. Land them all
//     and the enemy is gone for the rest of the attempt.
//   * a WAND never kills. One wave freezes an enemy where it stands, for
//     several seconds -- longer with strength. It always works, and it never
//     solves anything.
//
// Neither is better. The sword is an investment and the wand is instant
// relief, so a weak creature is better off waving and a strong one is better
// off swinging -- which makes the weapon interact with the points budget
// instead of going round it. That was the whole condition for letting a
// cosmetic choice become a real one; see docs/adr/0016.
//
// v1 and v2 are untouched and still shipped.
//
// v2 existed because v1's enemies could walk through walls. An enemy that gave
// chase left the corridor it was drawn in, and when it lost interest it went
// back to pacing the corridor's LENGTH at whatever column the chase had left it
// in -- so it marched up and down through solid rock, forever. v1 also checked
// an enemy's clearance at a single point while checking the player's across its
// whole body, so an enemy could stand half inside a wall even without chasing.
//
// v1 is untouched and still shipped: hard rule 3, and every link that pinned
// roam/1 still replays exactly as it did. See docs/adr/0013.
//
// Enemies die (to a sword). A hit knocks one down; enough hits and it is gone
// for the rest of the attempt, the way every game of this shape works. Start
// the level again and the room is full again.
//
...
//
// What the creature's two characteristics do here:
//
//   * speed    -- how fast you walk, and so whether you can outrun a chase
//   * strength -- how few swings put an enemy down, how long a wand freezes
//                 one, and how many hits you can take
//
// Both mean the same things in the side-on engine. That is the point of the
// budget: a characteristic that carries over is worth spending on.
...
export const ROAM_V4_BEHAVIOUR = 4;
...
export class RoamV4 implements Engine {
  readonly id = "roam" as const;
  readonly behaviourVersion = ROAM_V4_BEHAVIOUR;
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(`level has ${level.treasureCells.length} treasures; roam v4 holds ${MAX_TREASURE}`);
    }
    this.level = level;
