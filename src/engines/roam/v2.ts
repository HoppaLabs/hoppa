// Roam, behaviour version 2. Top-down, real time.
//
// v2 exists because v1's enemies could walk through walls. An enemy that gave
...
//
// v2 also lets enemies die. A hit knocks one down; enough hits and it is gone
...
//   * strength -- how hard you hit, how few hits an enemy takes to put down,
//                 how long it stays down, and how many hits you can take
//
...
export const ROAM_V2_BEHAVIOUR = 2;
...
export class RoamV2 implements Engine {
  readonly id = "roam" as const;
  readonly behaviourVersion = ROAM_V2_BEHAVIOUR;
  ...
  constructor(level: Level, creature: Creature = BRUK) {
    if (level.treasureCells.length > MAX_TREASURE) {
      throw new Error(`level has ${level.treasureCells.length} treasures; roam v2 holds ${MAX_TREASURE}`);
    }
    this.level = level;
